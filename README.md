...
* Functionally working:
    ...
    -  Terminal clears after each successful input.
* Displaying clear information given if incorrect inputs are made.
...
3. __Coordinate guess__
    ...
    <br>
    If a mine is not present, a numerical number is printed in the grid and the attempt counter is reduced by 1. This number represents the number of mines in the adjacent 8 grid spaces, further adding clues to the whereabouts of the remaining mines.
    <br>
...
4. __Input validity__
    <br>
    If the user does not enter a grid number greater than or equal to 5 or less than or equal to 9, then an error message is printed to the terminal. The user is requested to re-enter a valid number.
    <br>
...
5. __Game End__
    <br>
    If the user successfully finds all the mines before the attempt counter reaches zero then a winning message is displayed.
    <br>
...
6. __Future additions__
    * Can add functionality, that when a cell is revealed and the contents equal 0, Then all the surrounding cells that equal 0 will also be revealed. The revealing of neighboring cells will stop when a numerical value greater than 1 is met. This will need thought of how to implement a recursive function and was not achievable in the time frame for this project.
    * As like a traditional minesweeper game, can add functionality to "flag" cells that the user thinks may be a mine. This can help add to the enjoyment of the game especially when the grid increases in size. This will involve adding a further level of data to the game logic.
    * The game terminal is pre-set and in the future, increasing the terminal size will help accommodate larger grids (greater than 9), thus allowing the user to select a more challenging layout.
    * At the moment, the user can only modify the grid size. A consideration for the future can be that the user also enters the number of attempts they start with to increase or decrease difficulty.
...
To try and get around this, I had to stop reprinting the rules and game title as these would not be cleared when the game was running, preventing duplication of print statements at the top of the terminal.
